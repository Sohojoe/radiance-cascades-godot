...
layout(set=0,binding=10,rgba32f)writeonly uniform image2D output_cascade_image;
layout(set=0,binding=11,rgba32f)readonly uniform image2D cascade_prev_image;

// uniform sampler2D in_RenderScene;
// uniform sampler2D in_DistanceField;
vec2 in_RenderExtent;
vec2 in_CascadeExtent;
float in_CascadeCount;
float in_CascadeIndex;
float in_CascadeLinear;
...
vec4 raymarch(vec2 point, float theta, probe_info info) {
	...
	for(float i = 0.0, df = 0.0, rd = 0.0; i < info.range; i++) {									// Loop for max length of interval (in event that ray SDF is near 0 for entire length of ray).
		...
		if (df <= EPS)  return vec4(tosrgb(imageLoad(input_image, ivec2(ray * in_RenderExtent)).rgb), 0.0);	
	}
...
vec4 merge(vec4 rinfo, float index, probe_info pinfo) {
	...
	vec4 interpolated = imageLoad(cascade_prev_image, ivec2(probeUVN1));
...
void main() {
    ...
    in_RenderExtent = pc.render_extent;
    in_CascadeExtent = pc.cascade_extent;
    in_CascadeCount = pc.cascade_count;
    in_CascadeIndex = pc.cascade_index;
    in_CascadeLinear = pc.cascade_linear;
    in_CascadeInterval = pc.cascade_interval;
    in_TexelCoord = fragCoord.xy / in_CascadeExtent;
	...
	if (in_CascadeIndex == 0.0)															// Only for cascade0, apply sRGB conversion.
		fragColor = vec4(pow(fragColor.rgb, vec3(1.0 / 2.2)), 1.0);				// sRGB apporximation.
    imageStore(output_cascade_image,ifragCoord,fragColor);
}
