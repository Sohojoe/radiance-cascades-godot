...
//=======
// From Common
// const float MAX_FLOAT = uintBitsToFloat(0x7f7fffff);
...
void main() {
    ...
    d -= brushRadius * pc.iResolution.y;
    ...
    /*float t = 20.0f;
    sd = distance(fragCoord, vec2(0.66 + sin(t/4.0)/8.0, 0.5 + cos(t/4.0)/4.0) * pc.iResolution.xy) - 10.0;
    sd = min(sd, max(abs(fragCoord.x - 0.5 * pc.iResolution.x) - 10.0, abs(fragCoord.y - 0.5 * pc.iResolution.y) - 30.0));
    emissivity = (fragCoord.x < 0.52 * pc.iResolution.x) ? vec3(0.0) : vec3(4.0);*/
